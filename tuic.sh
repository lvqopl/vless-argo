...
install_tuic_server() {
    ...
    ARCH=$(uname -m)
    if [ "$ARCH" = "x86_64" ]; then
        DOWNLOAD_URL=$(curl -sL "https://api.github.com/repos/EAimTY/tuic/releases/latest" | grep "browser_download_url" | grep "x86_64-linux-gnu" | cut -d '"' -f 4)
    elif [ "$ARCH" = "aarch64" ]; then
        DOWNLOAD_URL=$(curl -sL "https://api.github.com/repos/EAimTY/tuic/releases/latest" | grep "browser_download_url" | grep "aarch64-linux-gnu" | cut -d '"' -f 4)
    else
    ...
    if [ -z "$DOWNLOAD_URL" ]; then
        echo -e "${RED}从 GitHub 获取 TUIC Server 下载链接失败。${NC}"
        exit 1
    fi
...
EOF
    #
    # 上面的 "users" 部分在 v5 中是可选的，并且 "token" 是主要的认证方式。这里保留 "users" 结构是为了兼容性，但实际不起作用。
    # 为了简化，我们只使用 token。将 `your-password` 替换为与 token 相同的值。
    sed -i "s/your-password/${CURRENT_TOKEN}/" $TUIC_CONFIG_FILE
...
modify_uuid() {
    ...
    sed -i "s/\"${CURRENT_UUID_OLD}\": \".*\"/\"${CURRENT_UUID}\": \"${CURRENT_TOKEN}\"/" $TUIC_CONFIG_FILE
...
show_main_menu() {
    ...
    update_status
    echo "======================================================"
    echo "         TUIC v5 协议一键部署管理脚本 v1.0.0"
    echo "======================================================"
